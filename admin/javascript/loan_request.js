...
const handle_approve_loan = async (btn, loan_request) => {
  ...
  try {
    const response = await fetch("/api/admin/loan_request/crud", {
      method: "POST",
...
const handle_disaprove_loan = async (btn, loan_request) => {
  ...
  btn.innerHTML = "Proccessing...";
  try {
    const response = await fetch("/api/admin/loan_request/crud/disaprove", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ token, admin, loan_request }),
    });
...
(async () => {
  ...
  let admin = getCookie("admin");
  try {
    const response = await fetch("/api/admin/loan_request", {
      method: "POST",
      headers: { "content-type": "application/json" },
